...
class ViewController: UIViewController {
    ...
    private func setupViews() {
        ...
        scoreLabel.configure(with:  ScoreLabel.ViewModel(crossesScore: crossesScore, zeroScore: crossesScore))
    }
    ...
    func elseWin() {
        if  hodLabel.text == "крестики выйграли" {
            scoreLabel.configure(with:  ScoreLabel.ViewModel(crossesScore: crossesScore+1, zeroScore: crossesScore))
        }else if  hodLabel.text == "нолики выйграли" {
            scoreLabel.configure(with:  ScoreLabel.ViewModel(crossesScore: crossesScore, zeroScore: crossesScore+1))
        }
    }
    func swithLabel() {
        switch currentStep {
    ...
    @objc func buttonRestartFunc() {
        ...
        swithLabel()
        count = 0
        imageCrosses.image = UIImage(systemName: "xmark")
        imageZero.image = UIImage (systemName: "circle.dashed")
    }
    
    @objc func buttonChangeFunc() {
        if count == 0 || count % 2 != 0 {
            imageCrosses.image = UIImage(systemName: "circle.dashed")
            imageZero.image = UIImage (systemName: "xmark")
            count = count + 1
        } else {
            imageCrosses.image = UIImage(systemName: "xmark")
            imageZero.image = UIImage (systemName: "circle.dashed")
